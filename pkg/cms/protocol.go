...
// newHash returns the hash function of this signer's digest algorithm, which is
// a function of the SHA-2 family or a Streebog one. Streebog has no crypto.Hash
// of its own, so the signer works with the hash.Hash interface here.
func (si signerInfo) newHash() (func() hash.Hash, error) {
	if streebog := streebogHashFor(si.DigestAlgorithm.Algorithm); streebog != nil {
		return streebog, nil
	}
	hash, err := si.hash()
	if err != nil {
...
// signingTime returns the time carried by the signing-time signed attribute, or
// the zero time when the attribute is absent.
func (si signerInfo) signingTime() (time.Time, error) {
	vals, err := si.SignedAttrs.values(oidAttrSigningTime)
	if err != nil {
	...
	if len(vals) != 1 || len(vals[0]) != 1 {
		return time.Time{}, ASN1Error{Message: "unexpected signing time attribute"}
	}

	var signingTime time.Time
	rest, err := asn1.Unmarshal(vals[0][0].FullBytes, &signingTime)
	if err != nil {
		return time.Time{}, err
	}
	if len(rest) > 0 {
		return time.Time{}, errTrailingData
	}
...
// maxBERDepth is the deepest nesting of constructed values that berToDER
// accepts. Encoding/asn1 bounds its own nesting in a similar way; the limit
// keeps a deeply nested input from exhausting the stack while it is being
// transcoded.
...
func berToDER(ber []byte) ([]byte, error) {
	...
	der, _, err := transcodeElement(ber, 0)
	if err != nil {
		return nil, err
	}
...
// the unread remainder of ber. depth is the nesting level of the element, which
// is capped at maxBERDepth.
func transcodeElement(ber []byte, depth int) (der, rest []byte, err error) {
	if depth > maxBERDepth {
		return nil, nil, ASN1Error{Message: "maximum nesting depth exceeded"}
	}
	...
	if indefinite {
		for {
			...
			var child []byte
			if child, rest, err = transcodeElement(rest, depth+1); err != nil {
				return nil, nil, err
			}
		...
		for len(body) > 0 {
			var child []byte
			if child, body, err = transcodeElement(body, depth+1); err != nil {
				return nil, nil, err
			}
			content = append(content, child...)
		}
