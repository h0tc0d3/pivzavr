...
const (
	...
	// timestampRequestTimeout is the longest a timestamp request may take,
	// including the time spent reading the response body.
	timestampRequestTimeout = 30 * time.Second
	// timestampMaxResponseSize is the largest timestamp response that is
	// accepted. A timestamp token is only a few kilobytes, so this leaves
	// ample room for a token with a long certificate chain while protecting
	// against a runaway response.
	timestampMaxResponseSize = 1 << 20
)
...
//
// The request runs against a client with a timeout and the response body is
// read through a size limit, so that a timestamping service that is slow, hangs
// or answers with a runaway body cannot block or exhaust the process.
func (req timeStampReq) do(url string) (timeStampResp, error) {
	...
	client := &http.Client{Timeout: timestampRequestTimeout}
	...
	if httpResp.StatusCode != http.StatusOK {
		return nilResp, fmt.Errorf("cms: unexpected timestamp response status %s", httpResp.Status)
	}
	...
	body, err := io.ReadAll(io.LimitReader(httpResp.Body, timestampMaxResponseSize+1))
	if err != nil {
		return nilResp, err
	}
	if len(body) > timestampMaxResponseSize {
		return nilResp, fmt.Errorf("cms: timestamp response exceeds %d bytes", timestampMaxResponseSize)
	}
