...
func FormatCertificate(output *CertificateOutput) string {
	...
	b.WriteString(i18n.Sprintf("Slot:          %s (%s)\n", output.Slot, output.Slot.Description()))
	b.WriteString(i18n.Sprintf("Fingerprint:   %s\n", output.Fingerprint))
	if output.Certificate != nil {
		cert := output.Certificate
		b.WriteString(i18n.Sprintf("Serial Number: %x\n", cert.SerialNumber))
		b.WriteString(i18n.Sprintf("Key Type:      %s\n", CertKeyType(cert)))
		b.WriteString(i18n.Sprintf("Not Before:    %s\n", formatTime(cert.NotBefore)))
		b.WriteString(i18n.Sprintf("Not After:     %s\n", formatTime(cert.NotAfter)))
		b.WriteString(i18n.Sprintf("Subject DN:    %s\n", formatName(cert.Subject)))
		b.WriteString(i18n.Sprintf("Issuer DN:     %s\n\n", formatName(cert.Issuer)))
	}
...
// type (e.g. "RSA 2048", "ECDSA NIST P-256", "GOST R 34.10-2012 256").
func CertKeyType(cert *x509.Certificate) string {
	...
	// A GOST R 34.10-2012 key is not one of the types that crypto/x509 knows,
	// so the certificate parses with an unknown public key algorithm and the
	// key is read from its subjectPublicKeyInfo instead.
	if cert.PublicKeyAlgorithm == x509.UnknownPublicKeyAlgorithm {
		if key, err := gost.ParsePublicKey(cert.RawSubjectPublicKeyInfo); err == nil {
			return fmt.Sprintf("GOST R 34.10-2012 %d (%s)", key.Curve.KeySize*8, key.Curve.Name)
		}
	}
...
// RFC 4514-style distinguished name string, and control characters such as line
// breaks, which would otherwise break the line structure of the output the name
// is rendered into.
func escapeNameValue(value string) string {
	...
	for i, c := range value {
		...
		case c < 0x20 || c == 0x7f:
			// Render control characters in the RFC 4514 hexadecimal form so
			// that a crafted attribute value cannot inject line breaks.
			escaped = append(escaped, []rune(fmt.Sprintf("\\%02X", c))...)
		default:
			escaped = append(escaped, c)
		}
	}
...
func unavailableIfEmpty(value string) string {
	if value == "" {
		return i18n.Sprintf("(unavailable)")
	}
	return value
}
...
func FormatDeviceInfo(info *DeviceInfo) string {
	var b strings.Builder
	b.WriteString(i18n.Sprintf("Name:        %s\n", info.Name))
	b.WriteString(i18n.Sprintf("Module:      %s\n", unavailableIfEmpty(info.Module)))
	b.WriteString(i18n.Sprintf("Firmware:    %s\n", info.FirmwareVersion))
	b.WriteString(i18n.Sprintf("Serial:      %s\n", info.SerialNumber))
	b.WriteString(i18n.Sprintf("CHUID:       %s\n", unavailableIfEmpty(info.CHUID)))
	b.WriteString(i18n.Sprintf("CCC:         %s\n", unavailableIfEmpty(info.CCC)))
	b.WriteString(i18n.Sprintf("PIN Retries: %s\n", formatRetries(info.PinRetries)))
	b.WriteString(i18n.Sprintf("PUK Retries: %s\n", formatRetries(info.PukRetries)))
